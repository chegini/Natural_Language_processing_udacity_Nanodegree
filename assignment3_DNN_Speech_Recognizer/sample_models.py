...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for l in range(1, recur_layers):
        simple_rnn = GRU(units, activation='relu',
                         return_sequences=True, implementation=2, name='rnn'+str(l))(bn_rnn)
...
def final_model(input_dim, units, recur_layers, dropout, output_dim=29):
    ...
    bn_rnn = BatchNormalization()(simple_rnn)
    # Add Dropout
    drop_rnn = Dropout(rate=dropout)(bn_rnn)
    
    for layer in range(1, recur_layers):
        simple_rnn = GRU(units, activation='relu',
                         return_sequences=True, implementation=2, name='rnn'+str(layer))(drop_rnn)
        # Add batch normalization
        bn_rnn = BatchNormalization()(simple_rnn)
        # Add Dropout
        drop_rnn = Dropout(rate=dropout)(bn_rnn)
